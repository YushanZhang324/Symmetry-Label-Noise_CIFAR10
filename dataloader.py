...
class cifar10Nosiy(datasets.CIFAR10):
    def __init__(self, root, train=True, transform=None, target_transform=None, download=False, nosiy_rate=0.0, asym=False):
        ...
        if asym:
            ...
            class_index = [np.where(np.array(self.train_labels) == i)[0] for i in range(10)]
            ...
            for i in noisy_idx:
                self.train_labels[i] = other_class(n_classes=10, current_class=self.train_labels[i])
            print(len(noisy_idx))
            print("Print noisy label generation statistics:")
            for i in range(10):
                n_noisy = np.sum(np.array(self.train_labels) == i)
