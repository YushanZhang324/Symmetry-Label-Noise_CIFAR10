...
trainset = cifar10Nosiy(root='./data',
                        train=True,
...
def test(epoch):
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/cross-entropy_noisecifar.pth')
        best_acc = acc
    
    #SummaryWriter
    writer.add_scalar('Test/Average loss', test_loss, epoch)
    writer.add_scalar('Test/Accuracy', acc, epoch)
    #writer.add_scalar('Test/Average loss', test_loss / len(trainloader.dataset), epoch)
    #writer.add_scalar('Test/Accuracy', correct / len(trainloader.dataset), epoch)


writer = SummaryWriter('log/tensorboard/cross-entropy_noisecifar')
